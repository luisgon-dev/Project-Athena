<script lang="ts">
	...
	async function loadSettings() {
		...
		try {
			...
			error =
				loadError instanceof Error ? loadError.message : 'Prowlarr settings failed to load.';
		} finally {
			loading = false;
		}
	}
	...
	async function saveSettings() {
		...
		try {
			...
			error =
				saveError instanceof Error ? saveError.message : 'Prowlarr settings failed to save.';
		} finally {
			saving = false;
		}
	}
	...
	async function testConnection() {
		...
		try {
			...
			error =
				testError instanceof Error ? testError.message : 'Prowlarr connection test failed.';
		} finally {
			testing = false;
		}
	}
...
<div class="space-y-6">
	<div class="dashboard-card">
		...
		<p class="mt-2 max-w-3xl text-sm leading-6 text-stone-600">
			...
			indexers into Athena through the Readarr-compatible application sync surface.
		</p>
	</div>
		...
		>
			<div class="grid gap-4 md:grid-cols-2">
				...
				<label class="space-y-2 md:col-span-2">
					...
					<input bind:value={apiKey} class="input-shell" placeholder={hasApiKey ? 'Stored API key will be kept unless replaced' : 'Enter Prowlarr API key'} type="password" />
				</label>
				<label class="flex items-center gap-3 rounded-[1.35rem] bg-stone-100/90 px-4 py-4">
